...
namespace ComLib.Lang.Extensions
{
    ...
    // FLUENT CALLS
    // Example 1 : method class
    activate user         
     
    // Example 2 : method class param1
    delete file c:\temp.txt.     
    
    // Example 3 : class param1 method
    file c:\temp.txt exists.
     
    // Example 4 : class method param1, named param2, named param3
    ...
    // 5. orderToBuy( shares(300), "IBM", 40.50, new Date(2012, 3, 30), "premium policy")
    </doc:example>
    ***************************************************************************/
    /// <summary>
    ...
    public class FluentMemberPlugin : ExprPlugin
    {
        ...
        private readonly IDictionary<Token, bool> ExpFluentFuncExpParenEnd = new Dictionary<Token, bool>()
        {
            { Tokens.Comma, true },
            { Tokens.Semicolon, true },
        ...
        public override bool CanHandle(Token current)
        {
            ...
            // Case 1: user.
            // Case 2: user(
            if (!(next.Token is IdToken)) return false;
        ...
        private Tuple<Type, Expr>  Match(string groupName, FluentPart part)
        {
            ...
            for (int ndx = 0; ndx < group.Count; ndx++)
            {
                ...
                if (type != null)
                {
                    ...
                    {
                        expr = result.Item2;
                        _tokenIt.Advance(lastTokenPeek);
                        break;
                    }
                }
            }
    ...
    public class FluentPluginHelper
    {
        ...
        {
            if (token is LiteralToken) return true;
            if (token is IdToken && _ctx.Symbols.IsVar(token.Text)) return true;
